...
export class OpenSeaPort {
  ...
  public async getFulfillOrderParameters(
      { order, accountAddress, recipientAddress, referrerAddress }:
      { order: Order;
        accountAddress: string;
        recipientAddress?: string;
        referrerAddress?: string; }
    ): Promise<{args: any, txnData: any}> {
    const matchingOrder = this._makeMatchingOrder({
      order,
      accountAddress,
      recipientAddress: recipientAddress || accountAddress
    ...
    const metadata = this._getMetadata(order, referrerAddress)
    const params = await this._atomicMatchParams({ buy, sell, accountAddress, metadata })
  ...
  private async _atomicMatchParams(
      { buy, sell, accountAddress, metadata = NULL_BLOCK_HASH }:
      { buy: Order; sell: Order; accountAddress: string; metadata?: string }
    ) {
    let value
